...
class StandardConsumer[T](bodyConsumer: Codec => BodyConsumer[T])(implicit execContext: ExecutionContext) extends StatusConsumer[Retryable[T]] {
  ...
  private def success(status: Status): Either[HeadersConsumer[Retryable[T]], Retryable[T]] = {
    ...
    status.code match {
      ...
      case 202 => Left(AcceptedHeadersConsumer)
    }
  }
...
sealed abstract class NewRequest {
  def body: JObject // TODO: Make this more precise (extract only the non-202-protocol related bits)
}
object NewRequest {
  def defaultRetryAfter = 60
}
case class Retry(body: JObject) extends NewRequest
case class Redirect(newUrl: String, inSeconds: Int,  body: JObject) extends NewRequest
case class RetryWithTicket(ticket: String, body: JObject) extends NewRequest
